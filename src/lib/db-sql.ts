...
export async function updateUser(id: number, name: string, password?: string) {
  if (password) {
    const hash = bcrypt.hashSync(password, 10)
    const [user] = await sql`
      UPDATE users SET name = ${name}, password = ${hash} WHERE id = ${id}
      RETURNING id, name, login, role, created_at as "createdAt"
    `
    return user || null
  }
...
export async function getStudents(groupId?: number) {
  ...
  return students.map(s => ({
    ...
    balance: s.balance, groupName: s.groupName,
...
export async function createStudent(name: string, phone: string, groupId: number) {
  const [s] = await sql`
    INSERT INTO students (name, phone, group_id, balance) VALUES (${name}, ${phone || ""}, ${groupId}, 0)
    RETURNING *
  `
  return { id: s.id, name: s.name, phone: s.phone, groupId: s.group_id, balance: s.balance, createdAt: s.created_at?.toISOString?.() || s.created_at }
}
...
export async function getStudent(id: number) {
  ...
  return s ? { ...s, groupId: s.group_id, createdAt: s.created_at?.toISOString?.() || s.created_at } : null
}
...
export async function createLesson(groupId: number, date: string, topic: string) {
  const [l] = await sql`
    INSERT INTO lessons (group_id, date, topic) VALUES (${groupId}, ${date}, ${topic || ""})
    RETURNING *
  `
...
export async function getPayments() {
  ...
  return payments.map(p => ({
    ...
    note: p.note || "", date: p.date, studentName: p.studentName,
...
export async function createPayment(studentId: number, amount: number, type: string, note: string, date: string) {
  const [p] = await sql`
    INSERT INTO payments (student_id, amount, type, note, date)
    VALUES (${studentId}, ${amount}, ${type}, ${note || ""}, ${date || new Date().toISOString().split("T")[0]})
    RETURNING *
  `
  if (type === "income") {
