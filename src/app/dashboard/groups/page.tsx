...
export default function GroupsPage() {
  ...
  function openCreate() {
    setEditId(null); setName(""); setDesc(""); setPrice(""); setDays([]); setShowForm(true); setError("")
  }

  function openEdit(g: any) {
    setEditId(g.id); setName(g.name); setDesc(g.description || ""); setPrice(g.pricePerLesson?.toString() || "")
    setDays(g.days ? g.days.split(",").filter(Boolean) : []); setShowForm(true); setError("")
  }

  async function save(e: React.FormEvent) {
    e.preventDefault(); setSaving(true); setError("")
    try {
      const body = { name, description: desc, pricePerLesson: parseInt(price) || 0, days: days.join(",") }
      if (editId) {
    ...
    <div className="animate-fadeIn">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400"><i className="fas fa-users mr-1" style={{ color: "var(--theme-primary)" }} />Barcha guruhlarni boshqaring</p>
        </div>
      ...
      {showForm && (
        <form onSubmit={save} className="bg-white p-4 lg:p-5 rounded-2xl shadow-sm border border-gray-100 mb-6 animate-slideIn">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
            <div>
              <label className="text-xs text-gray-400 block mb-1 font-medium"><i className="fas fa-tag mr-1" />Nomi</label>
            ...
            <div>
              <label className="text-xs text-gray-400 block mb-1 font-medium"><i className="fas fa-coins mr-1" />Dars narxi (so'm)</label>
              <input type="number" value={price} onChange={e => setPrice(e.target.value)} className="input-field" />
            </div>
          </div>
          <div className="mb-4">
            <label className="text-xs text-gray-400 block mb-2 font-medium"><i className="fas fa-calendar-week mr-1" />Hafta kunlari</label>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {groups.map(g => (
          <div key={g.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 card-hover animate-scaleIn relative overflow-hidden group">
            <div className="absolute top-0 right-0 w-32 h-32 rounded-full -mr-10 -mt-10 group-hover:scale-150 transition-transform duration-500" style={{ background: "var(--theme-primary)", opacity: 0.05 }} />
        ...
        {groups.length === 0 && (
          <div className="col-span-full py-16 text-center text-gray-400">
            ...
            <p className="text-sm mt-1">Yangi guruh qo'shish uchun <i className="fas fa-plus" /> tugmasini bosing</p>
          </div>
        )}
      </div>
    </div>
