...
export default function GroupsPage() {
  ...
  async function load() {
    ...
    if (!u) return
    fetch(`/api/groups?_=${Date.now()}`, { headers: authHeaders(false) }).then(r => r.json()).then(setGroups)
    if (u.role === "admin") fetch("/api/users", { headers: authHeaders(false) }).then(r => r.json()).then(setUsers)
  }
  useEffect(() => { load() }, [])
  ...
  async function save(e: React.FormEvent) {
    ...
    try {
      ...
      if (editId) {
        const res = await fetch("/api/groups", { method: "PUT", headers: authHeaders(), body: JSON.stringify({ id: editId, ...body }) })
        if (!res.ok) { const d = await res.json(); setError(d.error || "Xatolik"); setSaving(false); return }
      } else {
        const res = await fetch("/api/groups", { method: "POST", headers: authHeaders(), body: JSON.stringify(body) })
        if (!res.ok) { const d = await res.json(); setError(d.error || "Xatolik"); setSaving(false); return }
      }
  ...
  async function del(id: number) {
    ...
    await fetch(`/api/groups?id=${id}`, { method: "DELETE", headers: authHeaders(false) }); load()
  }
  ...
  async function toggleExpand(gid: number) {
    ...
    setExpandedId(gid)
    const u = getTokenUser()
    const all = await fetch(`/api/students?_=${Date.now()}`, { headers: authHeaders(false) }).then(r => r.json())
    ...
    <div className="animate-fadeIn">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
        <div className="page-header" style={{ marginBottom: 0 }}>
          ...
          <p className="page-desc"><i className="fas fa-users" style={{ color: "var(--theme-primary)" }} />{user?.role === "admin" ? "Barcha guruhlar" : "Sizning guruhlaringiz"}</p>
        </div>
        <button onClick={openCreate} className="btn-primary flex items-center gap-2 px-4 py-2.5 rounded-lg font-semibold text-sm cursor-pointer w-full sm:w-auto justify-center">
          <i className="fas fa-plus" /> Yangi guruh
        </button>
      </div>
      {showForm && (
        <form onSubmit={save} className="card p-4 lg:p-5 mb-6 animate-slideIn">
          {error && <div className="mb-3 p-3 bg-red-50 text-red-600 rounded-lg text-sm"><i className="fas fa-exclamation-circle mr-1" />{error}</div>}
            ...
            <div className="flex flex-wrap gap-2">
              {DAYS_UZ.map(d => (
                <button key={d} type="button" onClick={() => toggleDay(d)}
                  className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all cursor-pointer ${days.includes(d) ? "text-white shadow-lg scale-105" : "bg-gray-100 text-gray-500 hover:bg-gray-200"}`}
          ...
          <div className="flex gap-2">
            <button type="submit" disabled={saving} className="btn-primary px-4 py-2.5 rounded-lg font-semibold text-sm cursor-pointer flex items-center gap-2">
            ...
            <button type="button" onClick={() => setShowForm(false)} className="px-4 py-2.5 rounded-lg font-semibold text-sm bg-gray-100 text-gray-500 hover:bg-gray-200 cursor-pointer">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {groups.map(g => {
          ...
          return (
            <div key={g.id}>
              <div className="card p-5 card-hover animate-scaleIn relative overflow-hidden group cursor-pointer"
                ...
                <div className="relative">
                  ...
                  <div className="flex flex-wrap gap-2 text-sm">
                    <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg" style={{ background: "color-mix(in srgb, var(--theme-primary) 10%, transparent)" }}>
                  ...
                  <button onClick={(e) => { e.stopPropagation(); toggleExpand(g.id) }} className="mt-3 w-full py-2 rounded-lg text-sm font-semibold transition-all cursor-pointer flex items-center justify-center gap-2"
              ...
              {expandedId === g.id && (
                <div className="card rounded-t-none border-t-0 p-4 mb-2 animate-slideIn">
                  ...
                  {groupStudents.length === 0 ? (
                    ...
                    <div className="space-y-1.5">
                      {groupStudents.map((s: any) => (
                        <div key={s.id} className="flex items-center justify-between rounded-lg px-3 py-2 border border-gray-100 bg-white">
