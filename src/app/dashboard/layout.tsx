...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const links = [
    { href: "/dashboard", label: "Dashboard", icon: "fa-chart-simple" },
    { href: "/dashboard/groups", label: "Guruhlar", icon: "fa-users" },
    { href: "/dashboard/students", label: "O'quvchilar", icon: "fa-user-graduate" },
    { href: "/dashboard/attendance", label: "Davomat", icon: "fa-check-circle" },
    { href: "/dashboard/payments", label: "To'lovlar", icon: "fa-credit-card" },
    { href: "/dashboard/lessons", label: "Darslar", icon: "fa-book" },
    { href: "/dashboard/users", label: "Foydalanuvchilar", icon: "fa-user" },
  ]
    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 flex flex-col p-6 text-white transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
        ...
        <div className="flex items-center gap-3 mb-8">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center font-bold" style={{ background: `linear-gradient(135deg, var(--theme-primary), var(--theme-secondary))` }}>
        ...
        <div className="relative mb-2">
          <button onClick={() => setShowTheme(!showTheme)}
            ...
            <i className="fas fa-palette text-base" /> Rang tanlash
